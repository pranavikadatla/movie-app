...
import { API_OPTIONS, OPENAI_KEY } from '../utilis/constants';
import { addGptMovieNames, addGptMovieResult } from '../utilis/gptSlice';
    ...
    <div>
      <div className='pt-[10%] flex justify-center '>
        <form className='bg-black w-1/2 grid grid-cols-12' onSubmit={(e) => e.preventDefault()}>
          <input className='px-2 py-3 my-3 mx-2 col-span-10 rounded-lg'
            value={searchText}
            type="text"
            placeholder='What would you like to Watch?'
            onChange={(e) => setSearchText(e.target.value)} />
          <button
            className=' col-span-2 my-3 mx-2 p-1 bg-blue-600 text-white text-lg rounded-lg'
      ...
      <div className=' p-4 m-4 mt-6 text-white bg-black bg-opacity-70'>
        <div className='flex justify-around flex-wrap'>
          {searchText ? (movieNames && movieNames.map(movieName => (
            <div className='my-2 w-60 ' key={movieName.id.videoId}>
