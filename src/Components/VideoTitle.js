import React from 'react'

const VideoTitle = ({title,overview}) => {
  return (
    <div className='w-screen aspect-video pt-10 md:pt-[11%] px-5  md:px-10  absolute text-white bg-gradient-to-r from-black'>
        <h1 className='text-4xl md:text-6xl font-bold '>{title}</h1>
        <p className=' hidden sm:hidden md:inline-block w-1/4 py-4 px-2 text-sm'>{overview}</p>
        <div className='flex'>
          <div className=''>
             {/* <img className="w-5 h-7 bg-slate-400 "
            src="data:image/png;base64,"/> */}
            <button className='hidden sm:hidden md:inline-block mx-2 text-md py-2 px-10  bg-white rounded-md  text-black'> Play</button>
        </div>
            <button  className='hidden sm:hidden md:inline-block mx-2   text-md py-2 px-6 bg-gray-500 rounded-md  text-white'> More Info</button>
      </div>

    </div>
  )
}

export default VideoTitle;